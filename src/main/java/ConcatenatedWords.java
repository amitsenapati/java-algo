...
//        String[] words = {"cat","cats","catsdogcats","dog","dogcatsdog","hippopotamuses","rat","ratcatdogcat"};
        ...
        System.out.println(concatenatedWords.wordBreak(s, wordDict));
    }
    ...
    public List<String> findAllConcatenatedWordsInADict(String[] words) {
        ...
        for(String word: wordList){
            if (word.equals("bilt")){
                System.out.println("Main loop : " + word);
            }
            List<String> matched = new ArrayList<>();
            for(String word1 : wordList){
                if (!(word.equals(word1)) && word.contains(word1)){
                    if (word.equals("bilt")){
                        System.out.println("Inner loop matched : " + word1);
                    }
                    matched.add(word1);
                }
            }
            List<String> finalList = matched;
//            List<String> finalList = new ArrayList<>();
//            for(String word3 : matched){
//                boolean selected = true;
//                for(String word4 : matched){
//                    if(!(word3.equals(word4)) && word4.startsWith(word3)){
//                        if (word.equals("lqdqrrcrwdnxeuo")){
//                            System.out.println("Remove from list : " + word3);
//                        }
//                        selected = false;
//                    }
//                }
...
//                }
//            }
//            finalList.sort((s1, s2) -> Math.abs((s1.length()) - Math.abs(s2.length())));
            finalList.sort(Comparator.comparing(String::length).reversed());
            if (word.equals("bilt")){
                System.out.println("Matched sub words : " + finalList.toString());
            }
//            System.out.println("Matched sub words : " + finalList.toString());
            String temp = word;
            for(String match : finalList){
                temp = temp.replace(match, "");
                if (word.equals("bilt")){
                    System.out.println("After replaement : " + temp);
                }
            }
            if (temp.length() == 0 && finalList.size() >= 2){
                result.add(word);
            }
        }
